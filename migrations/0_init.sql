...
CREATE TABLE `game_sessions` (
    ...
    `tracker_id` TEXT COLLATE BINARY,
    `created_at` INTEGER NOT NULL
);
...
CREATE TABLE `game_session_events` (
    ...
    `event_data` TEXT COLLATE BINARY,
    `created_at` INTEGER NOT NULL
);

-- table which stores cached, computed data of game session
-- player1_uuid means first place
-- player2_uuid means second place and so on
...
-- user_uid might be anything (i.e firebase uid, uuid, etc.)
